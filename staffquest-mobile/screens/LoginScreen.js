...
export default function LoginScreen({onLogin}){
  ...
  async function manualLogin(){
    ...
    await save(data);
    if(!isOffline()){
      await ensureUser(id, { name: user.name, role: user.role, xp: user.xp });
    }
    onLogin(user);
  }
  ...
  async function googleLogin(){
    try{
      ...
      if(response.type === 'success'){
        ...
        const user = { id: fbUser?.uid || ('u' + Date.now()), name: fbUser?.displayName || 'Google User', role: 'employee', xp: 0 };
        const data = await load();
        if(!data.users.find(u=>u.id===user.id)){ data.users.push(user); await save(data); }
        await ensureUser(user.id, { name: user.name, role: user.role, xp: user.xp });
        onLogin(user);
      }
    }catch(e){
      setError('Google sign-in failed (using demo config). You can still use manual login.');
    }
  }
  ...
  return (
    <View style={{flex:1, backgroundColor:'#0b0d10', padding:20, justifyContent:'center'}}>
      ...
      <TextInput value={name} onChangeText={setName} placeholder="Your name" placeholderTextColor="#7a8599"
